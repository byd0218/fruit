...
<html >
  ...
  <body>
      ...
      <div style="float:right;font-size:10px;">创建日期：${contract.ctime}</div>
          合同编码：<b style="color:blue">${contract.barcode}</b><br/>
          类型：<c:if test="${contract.type==1}">省内</c:if>
           <c:if test="${contract.type==2}">省外</c:if><br/>
      <hr/>
      <div class="info">
                   零售商信息：<br/>
      ...
      <div class="info">
                货物信息：<br/>
         <c:if test="${commodityVOList!=null}">
