...
<html >
  ...
  <body onload="init()">
   ...
   <div class="addMask">
	   <div class="c">
	     ...
	     <form id="addForm"
               action="${pageContext.request.contextPath}/commodities/addCommodities" method="post" onsubmit="checkAddRetailer()">
		        名称：<input type="text" id="addName" name="name" style="width:120px"/> <br/>
		        产地：<input type="text" id="addTelphone" name="locality" style="width:120px"/><br/>
		        价格：<input type="text" id="addAddress" name="price" style="width:120px"/><br/>
      ...
      <div class="updateMask">
          <div class="c">
              ...
              <form id="updateForm"
                    action="${pageContext.request.contextPath}/commodities/updateCommodities"
                    method="post" onsubmit="checkAddRetailer()">
                  名称：<input type="text" id="updateName" name="name" style="width:120px"
                            value=""/> <br/>
                  产地：<input type="text" id="updateLocality" name="locality"
  ...
  <form id="listForm" action="${pageContext.request.contextPath}/commodities/findCommodities"
        method="post">
  ...
  <c:if test="${commoditiesList!=null}">
	  <table style="margin-top: 10px;width:700px;text-align:center;" border=1>  
	    <tr>  
	      <td>序号</td><td>名称</td><td>价格</td><td>产地</td>
	      <td>创建日期</td><td>操作</td>
	   </tr>  
      <c:forEach items="${commoditiesList}" var="item" varStatus="status">
	     <tr>  
	       <td>${status.index+1}</td><td>${item.name }</td>
	       <td>${item.price}</td><td>${item.locality }</td>  
	       <td>${item.ctime}</td>
	       <td>
               <%--<button onclick="showUpdateMask('true')"
                       style="background-color:#173e65;color:#ffffff;width:70px;">更新</button>|--%>
	       		<a href="#"
